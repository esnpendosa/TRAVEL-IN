...
$query = "SELECT k.id, k.nama_tempat, k.kategori, k.lokasi, k.harga, k.deskripsi, k.jam_operasional, k.menu_khas, k.gambar, k.status, u.nama_lengkap AS mitra
          FROM kuliner k
...
<head>
    ...
    <title>Validasi Kuliner - Rencana-IN</title>
...
<body>
    ...
    <div class="main">
        ...
        <div class="table-container">
            <table id="tabel-kuliner" class="styled-table">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Gambar</th>
                ...
                <tbody>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td>
                                <img src="../asset/kuliner/<?= htmlspecialchars($row["gambar"]); ?>" alt="Gambar Kuliner" width="75">
                            </td>
                            ...
                            <td><?= htmlspecialchars($row['menu_khas']) ?></td>
                            <td>
                                <span class="badge badge-warning"><?= htmlspecialchars($row['status']) ?></span>
                            </td>
                            <td style="white-space: nowrap;">
                                <a href="validasi_kuliner.php?id=<?= $row['id'] ?>&action=approve" class="btn btn-success btn-approve">Approve</a>
                                <a href="validasi_kuliner.php?id=<?= $row['id'] ?>&action=reject" class="btn btn-danger btn-reject">Reject</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
        ...
        document.querySelectorAll('.btn-approve').forEach(button => {
            button.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    text: 'Apakah Anda yakin ingin menyetujui kuliner ini?',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
        ...
        document.querySelectorAll('.btn-reject').forEach(button => {
            button.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    text: 'Apakah Anda yakin ingin menolak kuliner ini?',
                    icon: 'error',
                    showCancelButton: true,
